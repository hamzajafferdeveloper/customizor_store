...
const Orders = ({ orders, store }: Props) => {
    ...
    const breadcrumbs: BreadcrumbItem[] = [{ title: 'All Orders', href: `/${store.id}/order/index` }];
                        ...
                        <TableBody>
                            {filteredOrders.map((order) => (
                                <TableRow key={order.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                    <TableCell><Link href={`/${store.id}/order/show/${order.id}`}>{order.product.title}</Link></TableCell>
